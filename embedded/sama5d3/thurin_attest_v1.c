...
#define TRUSTED_DISPLAY_MAX_MESSAGE 76
...
#define THURIN_STATEMENT_PREFIX "I control the Ethereum address: 0x"

static int thurin_statement_is_valid(const char *message)
{
    const size_t prefix_len = sizeof(THURIN_STATEMENT_PREFIX) - 1;
    const size_t expected_len = prefix_len + 40;

    if (!message)
        return 0;

    if (strlen(message) != expected_len)
        return 0;

    if (memcmp(message, THURIN_STATEMENT_PREFIX, prefix_len) != 0)
        return 0;

    for (size_t i = prefix_len; i < expected_len; i++) {
        char c = message[i];
    ...
    return text_is_renderable(message);
}

static int fingerprint_hex_to_bytes(
    const char fp[40],
    uint8_t out[OPENPGP_V4_FINGERPRINT_LEN])
{
...
static void show_prompt(const char *fp, const char *message)
{
    ...
    size_t line1_len = message_len > 38 ? 38 : message_len;
    size_t line2_len = message_len - line1_len;

    if (line2_len > 38)
        line2_len = 38;

    memcpy(fp_line1, fp, 20);
    fp_line1[20] = '\0';

    memcpy(fp_line2, fp + 20, 20);
    fp_line2[20] = '\0';

    memcpy(msg_line1, message, line1_len);
    msg_line1[line1_len] = '\0';

    memcpy(msg_line2, message + line1_len, line2_len);
    msg_line2[line2_len] = '\0';

    clear_screen(BLACK);

    draw_text(48, 8, "OPENPGP SIGN", 2, WHITE);

    draw_text(84, 36, "SIGNING KEY", 1, WHITE);
    draw_text(60, 49, fp_line1, 1, WHITE);
    draw_text(60, 61, fp_line2, 1, WHITE);

    draw_text((W - (int)strlen(msg_line1) * 6) / 2,
              92, msg_line1, 1, WHITE);

    if (line2_len > 0) {
        draw_text((W - (int)strlen(msg_line2) * 6) / 2,
                  106, msg_line2, 1, WHITE);
    }
...
int main(void)
{
    ...
    if (!thurin_statement_is_valid(message)) {
        clear_screen(0x0000);
        draw_text(36, 105, "BAD THURIN MSG", 2, 0xF800);
        lcd_flush();

        fprintf(stderr,
                "Rejected request: expected exact Thurin Ethereum "
                "ownership statement format\n");
    ...
    printf("Requesting NeoPGP OpenPGP signature...\n");

    if (fingerprint_hex_to_bytes(fp, fingerprint) != 0) {
        fprintf(stderr, "Could not decode NeoPGP fingerprint\n");
        return 1;
    }

    memcpy(key_id,
           fingerprint + OPENPGP_V4_FINGERPRINT_LEN - sizeof(key_id),
           sizeof(key_id));

    time_t now = time(NULL);

    /*
     * The SAMA clock is initialized from the Mac during demo setup.
     * Fail closed if it was not initialized.
     */
    if (now < (time_t)1577836800 ||
        (uint64_t)now > 0xffffffffULL) {
        clear_screen(0x0000);
        draw_text(48, 105, "CLOCK ERROR", 2, 0xF800);
        lcd_flush();

        fprintf(stderr,
                "Refusing to sign: invalid board clock: %lld\n",
                (long long)now);
        return 1;
    }

    uint32_t creation_time = (uint32_t)now;
    ...
    if (openpgp_v4_build_sig_fields(
            ...
            &sig_fields_len) != 0) {
        fprintf(stderr,
                "Could not build OpenPGP signature fields\n");
        return 1;
    }
    ...
    printf("OpenPGP creation time: %u\n", creation_time);
    ...
    if (openpgp_v4_build_signature_packet(
            ...
            &signature_packet_len) != 0) {
        fprintf(stderr,
                "Could not build OpenPGP signature packet\n");
        return 1;
    }
